...
from summerizer_agent import summary_prompt_generating_agent
from summerizer_agent import new_summary_prompt_generating_agent
...
def generate_summary_new(model, input_dict):
    max_length = 1000
    chunks = divide_text_into_chunks(input_dict['topic'], max_length)
    total_summary = ""
    for chunck in chunks:
        print(f"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ {chunck}")
        input_text = f""" for the below part of story generate a summary 
        Story Part : {chunck}
    
                    """
        generated_summary = summary_prompt_generating_agent(input_text)
        total_summary += generated_summary
    return total_summary

def generate_summary(model, input_dict):

    input_text = f""" for the below novel/chapter generate a summary 
    summary : {input_dict['topic']}
 
                """
    generated_summary = new_summary_prompt_generating_agent(input_text)

    return generated_summary

def divide_text_into_chunks(input_text, max_words):
    """
    Divides the input_text into chunks of a specified maximum word count, ensuring
    the split occurs at the next newline character if possible.

    Parameters:
        input_text (str): The text to be divided.
    ...
    while start < len(words):
        ...
        # Join words to form a chunk and find the last newline if present
        chunk = " ".join(words[start:end])
        newline_index = chunk.rfind("\n")

        if newline_index != -1:
            # Adjust the end to include up to the newline
            adjusted_end = start + len(chunk[:newline_index].split())
            chunk = " ".join(words[start:adjusted_end])
            end = adjusted_end
...
if __name__ == "__main__":
    ...
    new_summary = generate_summary_new(model, input_dict)
    ...
    # decoded_summary = decode_nested(summary)
    # decoded_youtube_details = decode_nested(youtube_details)
    # decoded_story_elements = decode_nested(story_elements)
    # decoded_story = decode_nested(input_dict['topic']).replace("\n", " ")
    # summary_dict = {
    #     "story": decoded_story,
    #     "summary": decoded_summary,
    #     "youtube_details": decoded_youtube_details,
    #     "story_elements": decoded_story_elements,
    # }
    summary_dict = {
        "story" : input_dict['topic'],
        "summary": replace_special_characters(summary),
        "new_summary": new_summary,
        "new_summary_clean": replace_special_characters(new_summary),
